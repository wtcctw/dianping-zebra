...
import com.dianping.zebra.jdbc.util.CollectionUtils;
import com.dianping.zebra.parser.condition.ComparableAndInExpression;
import com.dianping.zebra.parser.condition.Comparative;
import com.dianping.zebra.parser.condition.Expression;
...
import com.dianping.zebra.parser.sqlParser.Update;
...
public class ShardColumnValueUtil {
	...
	private static Set<Object> eval(Expression expression, EvalContext context) {
		...
		if (expression instanceof ComparableAndInExpression) {
			Set<Comparative> evalSet = ((ComparableAndInExpression) expression).eval(context.tableAliases, context.column, context.params);
			/*
	...
	private static Set<Object> evalInsert(Insert insertSql, String column, List<Object> params) {
		...
		if (evalSet.isEmpty()) {
			throw new DataSourceRouteException("Router column[" + column + "] not found in insert sql[" + insertSql + "].");
		}
		return evalSet;
	}

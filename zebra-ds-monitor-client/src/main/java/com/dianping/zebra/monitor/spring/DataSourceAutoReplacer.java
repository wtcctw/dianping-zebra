...
public class DataSourceAutoReplacer implements BeanFactoryPostProcessor, PriorityOrdered {
	...
	private Set<PropertyValue> getGroupDataSourceProperties(BeanDefinition writeDsBean, DataSourceInfo outerDsInfo,
			boolean isFromDpdl) {
		...
		DataSourceInfo info = isFromDpdl ? getDataSourceInfo(writeDsBean, null) : outerDsInfo;
		if (isFromDpdl) {
			outerDsInfo.setDatabase(info.getDatabase());
			outerDsInfo.setUrl(info.getUrl());
			outerDsInfo.setUsername(info.getUsername());
			outerDsInfo.setType(info.getType());
		}

		if ("mysql".equals(info.getType()) && canReplace(info)) {
			//todo: read from canReplacedDatabase
			String groupConfig = LionUtil.getLionConfig(String.format("groupds.%s.mapping",
					isFromDpdl ? canReplacedDatabase.get(info.getDatabase()) : info.getDatabase() + ".single"));
			if (!StringUtils.isBlank(groupConfig)) {
				properties
						.add(new PropertyValue("jdbcRef", isFromDpdl ? info.getDatabase() : info.getDatabase() + ".single"));
	...
	private void processC3P0() {
		new DataSourceProcesser().process(c3p0Ds, new DataSourceProcesserTemplate() {
			...
			public void process(BeanDefinition dataSourceDefinition, String beanName, DataSourceInfo info) {
				Set<PropertyValue> properties = getGroupDataSourceProperties(dataSourceDefinition, info, true);

				if (properties.size() > 0) {
					replaceBeanPropertiesToDataSource(dataSourceDefinition, properties);
	...
	private void processDpdl() {
		new DataSourceProcesser().process(dpdlDs, new DataSourceProcesserTemplate() {
			...
			public void process(BeanDefinition dataSourceDefinition, String beanName, DataSourceInfo info) {
				...
				Set<PropertyValue> properties = getGroupDataSourceProperties(dpdlInnerDsBean, info, true);
	...
	private void replaceBeanPropertiesToDataSource(BeanDefinition dataSourceDefinition, Set<PropertyValue> properties) {

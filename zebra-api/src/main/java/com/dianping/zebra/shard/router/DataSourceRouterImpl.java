...
public class DataSourceRouterImpl implements DataSourceRouter {
    ...
    public RouterTarget getTarget(String sql, List<Object> params) throws DataSourceRouteException {
        try {
            ...
            if (tableShardRule != null) {
                ...
                boolean acrossTable =
                        dbAndTables.size() > 1 || dbAndTables.entrySet().iterator().next().getValue().size() > 1;
                target.setTargetedSqls(
                        createTargetedSqls(dbAndTables, acrossTable, sql, dmlSql, tableShardRule.getTableName(), skip,
                                max));
                ...
                if (dataSource == null) {
                    throw new DataSourceRouteException(
                            "Named datasource[name='" + DataSourceRepository.DEFAULT_DS_NAME + "'] not found.");
                }
    ...
    private String reconstructSqlLimit(boolean acrossTable, String sql, DMLCommon dmlSql, int skip, int max) {
        ...
        if ((dmlSql instanceof Select) && acrossTable && (max != RouterTarget.NO_MAX || skip != RouterTarget.NO_SKIP)) {
            if (((Select) dmlSql).getWhere().getGroupByColumns() != null && !((Select) dmlSql).getWhere()

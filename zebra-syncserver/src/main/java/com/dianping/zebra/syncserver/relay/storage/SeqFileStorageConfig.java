 ...
 * @author Leo Liang
 */
public class SeqFileStorageConfig {
	private static final Logger							log						= Logger.getLogger(SeqFileStorageConfig.class);
	...
	private static Map<String, SeqFileStorageConfig>	instanceMap				= new HashMap<String, SeqFileStorageConfig>();
	...
	private SeqFileStorageConfig(String configFile) {
		Properties prop = new Properties();
		InputStream in = null;
		try {
			in = SeqFileStorageConfig.class.getClassLoader().getResourceAsStream(configFile);
			prop.load(in);
		} catch (Exception e) {
	...
	public synchronized static SeqFileStorageConfig getInstance(String configFile) {
		if (instanceMap.get(configFile) == null) {
			instanceMap.put(configFile, new SeqFileStorageConfig(configFile));
		}

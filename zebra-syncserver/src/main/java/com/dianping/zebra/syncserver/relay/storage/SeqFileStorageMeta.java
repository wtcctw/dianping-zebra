 ...
 * @author Leo Liang
 */
public class SeqFileStorageMeta {
	...
	private static Logger			log			= Logger.getLogger(SeqFileStorageMeta.class);
	...
	public SeqFileStorageMeta(String storageName, String configFile) {
		super();
		this.meta = new Meta();
		String metaPath = SeqFileStorageConfig.getInstance(configFile).getMetaFilePath() + File.separator + storageName
				+ File.separator;
		...
		try {
			...
			throw new RuntimeException("Construct " + SeqFileStorageMeta.class.getCanonicalName() + " failed.", e);
		}
		loadFromDisk();
	}

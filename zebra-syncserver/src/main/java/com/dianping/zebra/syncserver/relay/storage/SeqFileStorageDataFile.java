 ...
 * @author Leo Liang
 */
public class SeqFileStorageDataFile {
	private final static Logger		log		= Logger.getLogger(SeqFileStorageDataFile.class);
	...
	public SeqFileStorageDataFile(String storageName, String configFile) {
		this.path = SeqFileStorageConfig.getInstance(configFile).getDataFilePath() + File.separator + storageName
				+ File.separator;
	...
	public synchronized void archiveAndRemoveOldFile(String filePath) {
		...
		for (int i = 0; i < subSetArray.length; i++) {
			...
			try {
				if (SeqFileStorageConfig.getInstance(configFile).isDataBakOn()) {
					SimpleDateFormat sf = new SimpleDateFormat("yyyyMMdd");

					String bakPath = SeqFileStorageConfig.getInstance(configFile).getDataBakPath() + File.separator

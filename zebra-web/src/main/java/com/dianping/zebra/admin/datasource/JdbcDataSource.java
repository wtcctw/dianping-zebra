...
/**
 * rewrite unidal-dal JdbcDataSource to support config from Lion.
 * 
 * @author damonzhu
 *
 */
public class JdbcDataSource implements DataSource, Initializable, LogEnabled, Disposable {
	...
	public void initialize() throws InitializationException {
		...
		try {
			...
			cpds.setJdbcUrl(getValueFromLion(c.getUrl()));
			cpds.setUser(getValueFromLion(c.getUser()));
			cpds.setPassword(getValueFromLion(c.getPassword()));
	...
	private String getValueFromLion(String key) {
		int pos = key.indexOf("$");
		...
		if (pos >= 0) {
			...
			String lionKey = key.substring(begin + 1, end);

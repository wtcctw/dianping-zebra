...
public class DalConfigServiceImpl implements DalConfigService {
	...
	private final String AUTO_REPLACE_KEY = "groupds.autoreplace.database";
			...
			List<String> keys = Lists.newArrayList(Iterables.filter(configs.keySet(), new Predicate<String>() {
				...
				public boolean apply(String input) {
					...
					if (StringUtils.isNotBlank(otherkey)
					      && input.matches("^ds\\." + otherkey + "[a-zA-Z0-9\\-]*\\.jdbc\\.[a-zA-Z0-9]+$")) {
						return true;
					}
	...
	public void updateDsConfig(GroupConfigModel modal, boolean isForce) {
		...
		try {
			...
			Cat.logEvent("GroupDsConfigUpdate",
			      String.format("env:%s key:%s from:%s to:%s", modal.getEnv(), groupKey, oldConfig, modal.getConfig()));
			...
			for (DsConfigModel ds : modal.getConfigs()) {
				for (ConfigProperty prop : ds.getProperties()) {
					if (prop.isDelete()) {
						...
						Cat.logEvent("DsConfigRemove", String.format("env:%s key:%s ", modal.getEnv(), prop.getKey()));
						continue;
					}
					if (prop.isCreate()) {
					...
					Cat.logEvent(
					      "DsConfigUpdate",
					      String.format("env:%s key:%s from:%s to:%s", modal.getEnv(), prop.getKey(), prop.getValue(),
					            prop.getNewValue()));
				}
			}
	...
	public Set<String> getWhiteList(String env) {
		...
		try {
			String value = m_lionHttpService.getConfigByHttp(env, AUTO_REPLACE_KEY);

			if (value != null) {
				String[] split = value.trim().split(",");

				for (String db : split) {
					whiteList.add(db.toLowerCase());
				}
			}
		} catch (IOException e) {
		}
